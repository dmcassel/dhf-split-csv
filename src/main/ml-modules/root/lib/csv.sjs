...
function CSVToArray(strData, strDelimiter) {
  ...
  // Use a do-while loop because the previous loop picked up the first piece of data we need
  do {
    // Get the delimiter that was found.
    let strMatchedDelimiter = arrMatches[1];
    // Check to see if the given delimiter has a length (is not the start of string) and if it matches field delimiter.
    ...
    if (
      ...
      if (Object.getOwnPropertyNames(curr).length > 0) {
        arrData.push(curr);
      }
      column = 0;
      curr = {};
    }
    // Now that we have our delimiter out of the way, let's check to see which kind of value we captured (quoted or
    // unquoted).
    let strMatchedValue = null;
    if (arrMatches[2]) {
      // We found a quoted value. When we capture this value, unescape any double quotes.
    ...
    curr[headers[column++]] = strMatchedValue;
  } while (arrMatches = objPattern.exec(strData));

  return arrData;
}

exports.split = CSVToArray;
